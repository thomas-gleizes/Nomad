...
//!
//! Un serveur no-border publie le service `_noborder._tcp.local.` avec, en TXT,
...
/// Type de service DNS-SD réservé à no-border.
pub const SERVICE_TYPE: &str = "_noborder._tcp.local.";
...
impl Registration {
    ...
    pub fn announce(node_id: NodeId, name: &str, port: u16) -> anyhow::Result<Registration> {
        ...
        let host = format!("nb-{node_id}.local.");
